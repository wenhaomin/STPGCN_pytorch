...
def str_config(config):
    str_cfg = (f"Data:{config.data.name} Spatio-Temporal Range:a{config.model.alpha}b{config.model.beta} Recent:{config.data.num_recent} History:{config.model.T} Predict:{config.model.T} Batch:{config.model.batch_size} lr:{config.model.learning_rate}\n"
